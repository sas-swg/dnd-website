(function () {
  ...
  function spellNames() {
    ...
    const add = (s) => {
      ...
      const lv = s.level || 0;
      (by[lv] = by[lv] || []).push(
        s.name + (s.concentration ? " (C)" : "") + (s.ritual ? " (R)" : ""),
      );
    };
    const __grantedMax =
  ...
  function featuresText(data) {
    ...
    if (data.ch && data.ch.fightingStyle && typeof FIGHTING_STYLES !== "undefined") {
      ...
      if (fst) a.push("• Fighting Style: " + fst.name);
    }
  ...
  function featuresDetailed(data) {
    ...
    if (data.ch && data.ch.fightingStyle && typeof FIGHTING_STYLES !== "undefined") {
      ...
      if (fst)
        a.push(
          "• Fighting Style: " + fst.name + (fst.description ? " — " + cut(fst.description) : ""),
        );
    }
    const bf = typeof backgroundFeatureText === "function" ? backgroundFeatureText() : "";
    if (bf) a.push("• " + cut(bf));
  ...
  function equipmentText(data) {
    ...
    if (
      ...
      out.push("Armor: " + ARMOR_DATA[inv.armor].name);
  ...
  async function generatePDF() {
    ...
    try {
      ...
      const SKILLS = [
        ["Acrobatics", "dex", "Acrobatics", "Check Box 23"],
        ["Animal", "wis", "Animal Handling", "Check Box 24"],
        ["Arcana", "int", "Arcana", "Check Box 25"],
        ["Athletics", "str", "Athletics", "Check Box 26"],
        ["Deception ", "cha", "Deception", "Check Box 27"],
        ["History ", "int", "History", "Check Box 28"],
        ["Insight", "wis", "Insight", "Check Box 29"],
        ["Intimidation", "cha", "Intimidation", "Check Box 30"],
        ["Investigation ", "int", "Investigation", "Check Box 31"],
        ["Medicine", "wis", "Medicine", "Check Box 32"],
        ["Nature", "int", "Nature", "Check Box 33"],
        ["Perception ", "wis", "Perception", "Check Box 34"],
        ["Performance", "cha", "Performance", "Check Box 35"],
        ["Persuasion", "cha", "Persuasion", "Check Box 36"],
        ["Religion", "int", "Religion", "Check Box 37"],
        ["SleightofHand", "dex", "Sleight of Hand", "Check Box 38"],
        ["Stealth ", "dex", "Stealth", "Check Box 39"],
        ["Survival", "wis", "Survival", "Check Box 40"],
      ];
      const pad = 2;
      function gp(n) {
        return FR[n];
      ...
      a.href = url;
      a.download = safeFileName(d.name) + "_Character_Sheet.pdf";
      ...
      if (label) label.innerText = "Download PDF";
    }
  }
