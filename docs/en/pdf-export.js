...
(function () {
     ...
     * ==================================================================== */
    // Lazy font loading (~1 MB): only fetched on the first PDF generation.
    ...
    function ensureDejavuFont() {
        ...
        if (!dejavuFontPromise) {
            dejavuFontPromise = new Promise((resolve, reject) => {
                ...
                s.onload = () => resolve();
                s.onerror = () => { dejavuFontPromise = null; reject(new Error("Failed to load dejavu-font.js")); };
    ...
    function generatePDF() {
        ...
        if (preLabel) preLabel.innerText = "Preparing font...";
        setPdfStatus("Loading PDF font...");
        ensureDejavuFont()
            .then(() => generatePDFCore())
            .catch((err) => {
                setPdfStatus(`Font failed to load: ${err.message}`, true);
                if (preBtn) preBtn.disabled = false;
                if (preLabel) preLabel.innerText = "Download PDF";
            });
    }
