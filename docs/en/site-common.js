(function () {
  ...
  // Inline SVG icons (Lucide) — consistent across OSes, unlike emojis
  ...
  function buildNav() {
    ...
    var __langLabel = __inEn ? "🇺🇦 UA" : "🇬🇧 EN";
    var __langTitle = __inEn ? "Switch to Ukrainian" : "Switch to English";
    var __langLink =
      '<a href="' +
      __langHref +
      '" data-lang-toggle class="hover:text-forest-accent transition-colors text-sm font-semibold" title="' +
      __langTitle +
      '">' +
      __langLabel +
      "</a>";
    var controls =
      ...
      'class="hover:text-forest-accent transition-colors inline-flex items-center" title="Search (press /)">' + ICONS.search + '</button>' +
    ...
    for (var j = 0; j < NAV_LINKS.length; j++) {
      ...
      mHtml +=
        '<a href="' +
        ml.href +
        '" class="py-2 border-b border-white/10 ' +
    ...
    mHtml +=
      '<a href="' +
      __langHref +
      '" class="py-2 border-b border-white/10 hover:text-forest-accent">' +
      __langLabel +
      "</a>";
    mHtml +=
      '<button type="button" data-search-open class="text-left py-2 border-b border-white/10 hover:text-forest-accent inline-flex items-center gap-2">' + ICONS.search + ' Search</button>';
    mHtml +=
      '<button type="button" data-theme-toggle-text class="text-left py-2 hover:text-forest-accent inline-flex items-center gap-2">' + ICONS.moon + ' Toggle theme</button>';
    mHtml += "</div>";
  ...
  function tokenScore(token, text) {
    ...
    if (idx !== -1) {
      var score = 60;
      if (idx === 0) score += 50;
      else if (/[\s(\[«"'\-]/.test(text.charAt(idx - 1))) score += 30;
  ...
  function buildSearchOverlay() {
    ...
    overlay.innerHTML =
      '<div id="global-search-box" role="dialog" aria-modal="true" aria-label="Search the database">' +
    ...
    // Escape closes search regardless of where focus is
    document.addEventListener("keydown", function (e) {
      if (e.key === "Escape" && overlay.classList.contains("open")) closeSearch();
    });
    // Focus-trap: Tab stays inside the open search dialog
    ...
    input.addEventListener("keydown", function (e) {
      ...
      if (e.key === "ArrowDown") {
        ...
        if (items[activeResult]) window.location.href = items[activeResult].getAttribute("href");
  ...
  function buildCatChips() {
    ...
    for (var j = 0; j < cats.length; j++) {
      html +=
        '<button type="button" class="gs-chip" data-cat="' +
        escapeHtml(cats[j]) +
        '">' +
        escapeHtml(cats[j]) +
        "</button>";
  ...
  function markTitle(title, tokens) {
    ...
    for (var m = 0; m < merged.length; m++) {
      out +=
        title.slice(pos, merged[m][0]) +
        "<mark>" +
        title.slice(merged[m][0], merged[m][1]) +
        "</mark>";
      pos = merged[m][1];
    }
    out += title.slice(pos);
  ...
  function renderResults(q, results) {
    ...
    for (var j = 0; j < limit; j++) {
      var m = scored[j].it;
      var url =
        m.page + "?find=" + encodeURIComponent(m.title || "") + (m.anchor ? "#" + m.anchor : "");
      html +=
        '<a class="gs-result" href="' +
        url +
        '">' +
        '<span class="gs-title">' +
        markTitle(m.title || "", tokens) +
        "</span>" +
        '<span class="gs-cat">' +
        escapeHtml(m.cat || "") +
        "</span></a>";
  ...
  function openSearch() {
    ...
    if (!overlay) return;
    // Accessibility: remember focus so we can restore it on close
  ...
  function openCollapsible(card) {
    ...
    if (content && !content.classList.contains("open")) {
      var header =
        card.querySelector('[onclick*="toggle"]') ||
  ...
  function focusSearchTarget() {
    ...
    if (!q) return;
    var nq = normalize(q);
    var primary = normalize(q.split(" (")[0]);
  ...
  function locateAndHighlight(nq, primary) {
    ...
    if (!target) {
      var els = document.querySelectorAll(
        "main h2, main h3, main h4, main td, main th, main li, main .nav-card",
      );
      var pick = null,
        pickLen = Infinity;
  ...
  function localizeSharedEnglish(root) {
    var dict={
      'Всі типи':'All types','Всі розміри':'All sizes','Усі типи':'All types','Створити власного монстра':'Create custom monster','🛠️ Створити власного монстра':'🛠️ Create custom monster',
      'Зберігається у цьому браузері та з’являється і в генераторі сутичок.':'Saved in this browser and also available in the encounter builder.','➕ Зберегти монстра':'➕ Save monster','ВАШІ МОНСТРИ':'YOUR MONSTERS','Ваші монстри':'Your monsters','Ще немає власних монстрів.':'No custom monsters yet.',
      'Аберація':'Aberration','Звір':'Beast','Небожитель':'Celestial','Конструкт':'Construct','Дракон':'Dragon','Елементаль':'Elemental','Фейрі':'Fey','Ісчадіє':'Fiend','Велетень':'Giant','Гуманоїд':'Humanoid','Потвора':'Monstrosity','Слиз':'Ooze','Рослина':'Plant','Нежить':'Undead',
      'Крихітний':'Tiny','Малий':'Small','Середній':'Medium','Великий':'Large','Величезний':'Huge','Гігантський':'Gargantuan',
      'Жезл':'Rod','Зілля':'Potion','Паличка':'Wand','Перстень':'Ring','Посох':'Staff','Сувій':'Scroll','Чудесний предмет':'Wondrous Item','Базова отрута':'Basic Poison','Спорядження пригодника':'Adventuring Gear',
      "Слов'янські (людина)":'Slavic (Human)','Західні (людина)':'Western (Human)','Ельфи':'Elves','Дворфи':'Dwarves','Напіврослики':'Halflings','Гноми':'Gnomes','Орки':'Orcs','Тіфлінги':'Tieflings','Драконороджені':'Dragonborn',
      '💾 Автозбереження':'💾 Autosave','📄 Аркуш':'📄 Sheet','⬆ Завантажити':'⬆ Import','🔄 Почати заново':'🔄 Start over'
    };
    var walker=document.createTreeWalker(root||document.body,NodeFilter.SHOW_TEXT);var nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);nodes.forEach(function(n){var raw=n.nodeValue,trim=raw.trim(),rep=dict[trim];if(!rep&&/^\d+ монстрів$/.test(trim))rep=trim.replace('монстрів','monsters');if(rep)n.nodeValue=raw.replace(trim,rep);});
    var attrs=(root||document).querySelectorAll? (root||document).querySelectorAll('[title],[placeholder],[aria-label]'):[];for(var i=0;i<attrs.length;i++)['title','placeholder','aria-label'].forEach(function(a){var v=attrs[i].getAttribute(a);if(v&&dict[v])attrs[i].setAttribute(a,dict[v]);});
  }
  function watchEnglishLocalization(){localizeSharedEnglish(document.body);var busy=false;new MutationObserver(function(ms){if(busy)return;busy=true;ms.forEach(function(m){for(var i=0;i<m.addedNodes.length;i++){var n=m.addedNodes[i];if(n.nodeType===1)localizeSharedEnglish(n);else if(n.nodeType===3){var t=n.nodeValue.trim();var holder=document.createElement('span');holder.textContent=t;localizeSharedEnglish(holder);if(holder.textContent!==t)n.nodeValue=n.nodeValue.replace(t,holder.textContent);}}});busy=false;}).observe(document.body,{childList:true,subtree:true});}
