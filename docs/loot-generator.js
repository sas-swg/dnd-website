...
const SOURCES = {
  chest:   { icon:'🧰', name:'Простий сундук', min:30, max:300,
    coins:.70, gems:.10, items:.20, magicChance:.15, maxItems:2,
    flavor:['Запилюжена скриня прихована під половицями.','Маленький кований сундук із іржавим замком.','Схованка дрібного злодія у стіні.'] },
  room:    { icon:'🚪', name:'Кімната підземелля', min:150, max:1500,
    coins:.50, gems:.20, items:.30, magicChance:.35, maxItems:3,
    flavor:['Покинута кімната з рештками табору.','Кам'+String.fromCharCode(39)+'яна зала зі старими скринями та полицями.','Сховище, де мешканці тримали ціннощі.'] },
  boss_room:{ icon:'👑', name:'Кімната після боса', min:2000, max:20000,
    coins:.40, gems:.35, items:.25, magicChance:.85, maxItems:5,
    flavor:['Скарбниця поверженого володаря підземелля.','Купи золота та скарбів навколо трону.','Захована скарбниця за потайними дверима.'] },
  boss:    { icon:'🐉', name:'Здобич із боса', min:500, max:8000,
    coins:.30, gems:.05, items:.65, magicChance:.90, maxItems:3, favorGear:true,
    flavor:['Спорядження, зняте з поверженого ворога.','Особисті речі та зброя боса.','Те, що бос носив при собі.'] }
};
...
const TYPE_LABELS = [
  ['weapon','⚡ Зброя'],['armor','🛡️ Броня'],['gear','🎒 Спорядження'],
  ['tool','🔨 Інструменти'],['potion','🧪 Зілля'],['ammo','🏹 Боєприпаси'],
  ['mount','🐎 Верхові'],['vehicle','🚐 Транспорт'],['poison','☠️ Отрути'],
  ['trinket','🪙 Дрібнички'],['magic','✨ Магічні']
];

// Самоцвіти та витвори мистецтва
const GEMS = {
  10:['азурит','кварц димчастий','гематит','малахіт','обсидіан','тигрове око','бірюза'],
  50:['багрянець','агат','кварц рожевий','місячний камінь','онікс','циркон'],
  100:['аметист','гранат','нефрит','перлина','шпінель','турмалін'],
  500:['топаз','аквамарин','чорна перлина','сапфір синій','спінель вогниста'],
  1000:['смарагд','рубін','сапфір зоряний','опал вогнистий','діамант жовтий'],
  5000:['діамант чистої води','чорний опал королівський','рубін голубиної крові']
};
const ART = {
  25:['срібна чаша','різьблена кістка','маленьке дзеркало в рамці','вишита шовкова хустка'],
  250:['золотий перстень з самоцвітом','різьблена нефритова статуетка','срібне намисто','корона з меді та срібла'],
  750:['золота брошка з перлами','малюнок відомого майстра','золота курильниця'],
  2500:['королівська корона','оздоблена самоцвітами коротка накидка','платиновий кубок'],
  7500:['діадема з самоцвітами','золотий скіпетр з рубінами','платинова статуетка бога']
};
...
function magicRarityIdx(it){ return RARITY_ORDER[it.rarityUk] || RARITY_ORDER[it.rarity] || 3; }
...
function buildSourceCards(){
  ...
  Object.entries(SOURCES).forEach(([key,s])=>{
    ...
    d.dataset.key = key;
    d.innerHTML = '<div class="text-2xl">'+s.icon+'</div><div class="font-bold text-sm text-forest mt-1">'+s.name+'</div><div class="text-[11px] text-slate-500">~'+fmtGold(s.min)+'–'+fmtGold(s.max)+' зол</div>';
...
function render(r){
  ...
  document.getElementById('totalValue').textContent = fmtGold(r.total)+' зол';

  // монети
  const coinBox = document.getElementById('coinsBox');
  const coinLabels = {pp:['ПМ','#e5e7eb'],gp:['ЗМ','#f59e0b'],sp:['СМ','#9ca3af'],cp:['ММ','#b45309']};
  const coinEntries = Object.entries(r.coins);
  if(coinEntries.length){
    coinBox.classList.remove('hidden');
    coinBox.innerHTML = '<div class="text-xs font-bold text-amber-700 uppercase mb-2">🪙 Монети</div><div class="flex flex-wrap gap-4">'+
  ...
  if(r.gems.length){
    gemBox.innerHTML = '<div class="text-xs font-bold text-slate-500 uppercase mb-2">💎 Самоцвіти та витвори мистецтва</div>'+
      '<div class="space-y-1">'+r.gems.map(g=>'<div class="flex justify-between items-center text-sm p-2 bg-slate-50 border border-slate-200 rounded-lg"><span>'+(g.art?'🎺 ':'💎 ')+g.name.charAt(0).toUpperCase()+g.name.slice(1)+'</span><span class="font-bold text-amber-600">'+fmtGold(g.value)+' зол</span></div>').join('')+'</div>';
  } else { gemBox.innerHTML=''; }

  // предмети
  const itemsBox = document.getElementById('itemsBox');
  if(r.items.length){
    itemsBox.innerHTML = '<div class="text-xs font-bold text-slate-500 uppercase mb-2">🎒 Предмети</div>'+
      '<div class="space-y-2">'+r.items.map((it,idx)=>{
        const idxR = it._isMagic ? magicRarityIdx(it) : 1;
        const rColor = RARITY_COLOR[idxR];
        const rName = it._isMagic ? (it.rarityUk||it.rarity||'') : (it.category||'Звичайний');
        const att = it.attunement ? ' · потребує налаштування' : '';
        return '<div class="loot-item flex items-start justify-between gap-2 p-3 border rounded-xl cursor-pointer '+rColor+'" onclick="showItem('+idx+')">'+
          '<div><div class="font-bold text-sm">'+(it._isMagic?'✨ ':'')+it.name+'</div>'+
          '<div class="text-[11px] opacity-80">'+rName+att+'</div></div>'+
          '<div class="text-right shrink-0"><div class="font-bold text-sm">'+fmtGold(it._price)+' зол</div></div></div>';
      }).join('')+'</div>';
  } else { itemsBox.innerHTML='<div class="text-sm text-slate-400 italic">Без предметів (лише монети/самоцвіти)</div>'; }
}
...
function showItem(idx){
  ...
  const idxR = it._isMagic ? magicRarityIdx(it) : 1;
  const rName = it._isMagic ? (it.rarityUk||it.rarity||'') : (it.category||'');
  document.getElementById('mMeta').innerHTML = '<span class="inline-block px-2 py-0.5 rounded border '+RARITY_COLOR[idxR]+'">'+rName+'</span>'+
    ...
    ' <span class="text-amber-600 font-bold">· '+fmtGold(it._price)+' зол</span>';
  document.getElementById('mDesc').textContent = it.description || it.descriptionEn || 'Опис недоступний.';
...
function copyResult(){
  ...
  const r = lastResult;
  let t = r.source.icon+' '+r.source.name+' ('+fmtGold(r.total)+' зол)\n'+r.flavor+'\n\n';
  const coinNames={pp:'платина',gp:'золото',sp:'срібло',cp:'мідь'};
  const coins=Object.entries(r.coins);
  if(coins.length){ t+='Монети: '+coins.map(([u,c])=>fmtGold(c)+' '+coinNames[u]).join(', ')+'\n'; }
  if(r.gems.length){ t+='Самоцвіти/мистецтво:\n'+r.gems.map(g=>'  • '+g.name+' ('+fmtGold(g.value)+' зол)').join('\n')+'\n'; }
  if(r.items.length){ t+='Предмети:\n'+r.items.map(i=>'  • '+i.name+' ['+(i._isMagic?(i.rarityUk||i.rarity):i.category)+'] — '+fmtGold(i._price)+' зол').join('\n')+'\n'; }
  navigator.clipboard.writeText(t).then(()=>{
    const b=document.getElementById('copyBtn'); const o=b.textContent; b.textContent='✅ Скопійовано'; setTimeout(()=>b.textContent=o,1500);
  });
}
...
function saveLoot(){
  if(!lastResult){ alert('Спочатку згенеруйте лут.'); return; }
  if(!window.DndStore) return;
  var def = lastResult.source.name + ' (' + fmtGold(lastResult.total) + ' зол)';
  var name = prompt('Назва для цього луту:', def);
  if(name===null) return;
  DndStore.save('loot', { type:'loot', name: (name||def).trim(), total: lastResult.total, result: JSON.parse(JSON.stringify(lastResult)) });
  renderSavedLoot();
  var b=document.getElementById('saveLootBtn'); if(b){var o=b.textContent; b.textContent='✅ Збережено'; setTimeout(function(){b.textContent=o;},1500);}
}
...
function deleteLoot(id){
  ...
  if(!confirm('Видалити «'+((rec&&rec.name)||'лут')+'»?')) return;
...
function renderSavedLoot(){
  ...
  items.forEach(function(rec){
    ...
    d.innerHTML='<div class="font-bold text-sm text-forest">'+_esc(rec.name)+'</div>'+
      '<div class="text-[11px] text-ui-muted mt-0.5">'+fmtGold(rec.total||0)+' зол • '+nItems+' предм. • '+nGems+' самоцв.</div>'+
      '<div class="mt-2 flex gap-2">'+
        '<button data-a="load" class="flex-1 py-1.5 bg-[#14532D] text-white rounded-lg text-xs font-bold hover:bg-[#166534]">▶ Відкрити</button>'+
